  ...
  return (
    <header
      className={cn(
        ...
        scrolled
          ? 'bg-cyber-darker/90 backdrop-blur-xl border-b border-cyber-border'
          : 'bg-transparent'
      )}
    >
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-20">
          ...
          <div className="hidden lg:block">
            <Link
              href="/enrollment"
              className="px-6 py-2.5 bg-gradient-to-r from-primary-600 to-primary-500 text-white text-sm font-semibold rounded-xl shadow-lg shadow-primary-500/25 transition-all hover:from-primary-500 hover:to-primary-400"
            >
              Записаться
            </Link>
          </div>
          ...
          <div className="px-4 py-6 space-y-2">
            {nav.map((i) => (
              <Link
                key={i.href}
                href={i.href}
                className="block px-4 py-3 text-gray-300 hover:text-white hover:bg-white/5 rounded-xl"
            ...
            <Link
              href="/enrollment"
              className="block mt-4 px-6 py-3 bg-gradient-to-r from-primary-600 to-primary-500 text-white text-center font-semibold rounded-xl"
              onClick={() => setMobileOpen(false)}
            >
              Записаться
            </Link>

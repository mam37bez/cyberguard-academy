  ...
  return (
    <section className="py-24 bg-cyber-dark relative">
      ...
      <div className="max-w-7xl mx-auto px-4 relative z-10">
        <SectionHeading badge="Отзывы" title="Что говорят ученики" subtitle="Реальные отзывы" />
        <div ref={ref} className={`grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 transition-all duration-700 ${isVisible?'opacity-100 translate-y-0':'opacity-0 translate-y-10'}`}>
          {testimonials.map(t=>(
            <Card key={t.id} variant="default">
              <CardContent>
                <div className="flex gap-1 mb-4">{Array.from({length:5}).map((_,i)=>(<span key={i} className={i<t.rating?'text-yellow-400':'text-gray-600'}>★</span>))}</div>
                <p className="text-gray-300 text-sm mb-6 italic">&ldquo;{t.content}&rdquo;</p>
                <div className="flex items-center gap-3">
                  <div className="w-10 h-10 rounded-full bg-gradient-to-br from-primary-500 to-cyber-green flex items-center justify-center text-white font-bold text-sm">{t.name.charAt(0)}</div>
